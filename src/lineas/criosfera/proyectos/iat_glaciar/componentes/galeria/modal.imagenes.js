...
export default function Modal(props){
    ...
    const back = (e)=>{
        e.preventDefault();
        if(state < listaImagenes.length){
            return setState(state + 1);
        }
    }
    const next = (e)=>{
        e.preventDefault();
        if(state > listaImagenes[0].id){
            return setState(state - 1)
        }
    }
    ...
    return(
        <div className='absolute h-screen top-0 z-40 w-full'>
            <div className='fixed flex justify-center h-screen w-full'>
                <div className='flex flex-col justify-start lg:justify-center md:justify-between lg:justify-center w-full opacity-image lg:py-10 h-screen'>
